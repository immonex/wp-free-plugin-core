...
class Video_Utils_Test extends WP_UnitTestCase {
	...
	public function test_embed_url() {
		...
		$this->assertEquals( $expected, ( "{$this->ns}\Video_Utils" )::get_oembed_url( $source ) );

		$this->assertEquals( $expected, ( "{$this->ns}\Video_Utils" )::get_oembed_url( $source, [ 'youtube-nocookie' => true ] ) );
		...
		$this->assertEquals( $expected, ( "{$this->ns}\Video_Utils" )::get_oembed_url( $source, [ 'time' => 60 ] ) );
	} // test_embed_url
