...
class Template_Utils {
	...
	public function render_twig_template( $filename, $template_data = array() ) {
		...
		try {
			$twig->getLoader()->setPaths( $template_folders );
			$output = $twig->render( basename( $filename ), $template_data );
		} catch ( \Exception $e ) {
			$output = false;
		}
	 ...
	 */
	public function render_twig_template_string( $template_string, $template_data = array() ) {
