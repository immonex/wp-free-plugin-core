...
class General_Utils {
	 ...
	 */
	public static function reset_post( $post_id, $taxonomies = false, $overwrite_defaults = [], $exclude_meta = [], $special_args = [] ) {
		global $user_ID;
		...
		$languages        = is_array( $active_languages ) && ! empty( $active_languages ) ? array_keys( $active_languages ) : [];
		...
		if (
			...
			pll_save_post_translations( [ $default_language => $post_id ] );
		}
		...
		$post_id = wp_update_post( array_merge( [ 'ID' => $post_id ], $post_defaults ) );

		do_action( 'inveris_base_after_post_reset', $post_id, $args );
		do_action( 'immonex_base_after_post_reset', $post_id, $args );
