...
class String_Utils {
	 ...
	 */
	public static function urlencode_special( $source ) {
		$entities     = [ '%21', '%2A', '%27', '%28', '%29', '%3B', '%3A', '%40', '%26', '%3D', '%2B', '%24', '%2C', '%2F', '%3F', '%25', '%23', '%5B', '%5D' ];
		$replacements = [ '!', '*', "'", '(', ')', ';', ':', '@', '&', '=', '+', '$', ',', '/', '?', '%', '#', '[', ']' ];

		return str_replace( $entities, $replacements, rawurlencode( $source ) );
	} // urlencode_special
	...
	public static function get_excerpt( $source_text, $length = 120, $suffix = '' ) {
		...
		foreach ( $words as $i => $word ) {
			...
			} elseif ( strlen( $text ) + 1 + strlen( $word ) <= $length ) {
				$text .= ' ' . $word;
			} else {
				break;
			}
	 ...
	 *                                 - zerodash: If true, trailing zeros of whole numbers will be replaced by a dash.
	 *                                       Alternatively, an arbitrary string can be specified to replace the trailing zeros.
	 ...
	 */
	public static function format_number( $value, $decimals = 2, $unit = '', $args = [] ) {
		global $wp_locale;
		...
		if ( $decimals && ! empty( $args['zerodash'] ) ) {
			$dash      = true === filter_var( $args['zerodash'], FILTER_VALIDATE_BOOLEAN ) ? '–' : $args['zerodash'];
			$formatted = preg_replace( "/({$dec_sep})0+$/", '$1' . $dash, $formatted );
		}
	 ...
	 * @return float Formatted number.
	 */
	public static function smooth_round( $value, $round_down = false, $smooth_zero_map = [] ) {
		$value = intval( $value );
		if ( empty( $smooth_zero_map ) || ! is_array( $smooth_zero_map ) ) {
			$smooth_zero_map = [ 0, 0, 1, 1, 2, 3, 3, 4 ];
		}
	...
	/**
	 * Check if the given URL belongs to the current WP installation (home or site URL).
	 *
	 * @since 2.2.0
	 *
	 * @param string          $url URL to check.
	 * @param bool            $case_sensitive True if the URL comparison shall be case-sensitive (optional).
	 * @param string|string[] $add_local_urls Additional local URL(s) to check against.
	 *
	...
	public static function is_local_url( $url, $case_sensitive = false, $add_local_urls = [] ) {
		...
		if ( is_multisite() ) {
			$local_urls[] = network_home_url();
			$local_urls[] = network_site_url();
		}

		if ( ! empty( $add_local_urls ) ) {
			if ( ! is_array( $add_local_urls ) ) {
				$add_local_urls = [ $add_local_urls ];
			}

			$local_urls = array_merge( $local_urls, $add_local_urls );
		}

		$local_urls = array_unique( $local_urls );

		foreach ( $local_urls as $local_url ) {
			$local_url_parts = wp_parse_url( $local_url );
			if ( false === $local_url_parts ) {
				continue;
			}
			...
			if ( ! $case_sensitive ) {
				$local_url_comp = strtolower( $local_url_comp );
			}

			if ( 0 === strpos( $url_comp, $local_url_comp ) ) {
				return true;
			}
		}
	 ...
	 */
	public static function is_virtual_tour_url( $url, $additional_search_terms = [] ) {
		...
		return 'http' === strtolower( substr( $url, 0, 4 ) ) && preg_match( '/(ogulo|immoviewer|matterport|mpskin|archilogic|feelestate|virtualtours\.immobilienscout24|immo\.tours|ricohtours|ricoh360|giraffe360' . ( $additional_search_terms ? '|' . $additional_search_terms : '' ) . ')/', strtolower( $url ) );
	} // is_virtual_tour_url
	...
	/**
	 * Determine a "safe" current URL, optionally with filtered query variables.
	 *
	 * @since 2.9.0
	 *
	 * @param string[] $query_var_names Names of query variables to include or exclude
	 *                                  (optional, default: empty, meaning all vars).
	 * @param string   $mode            "include" or "exclude" (optional, default: "include").
	 * @param string   $url_only        true (default) to return the URL string only, false for an
	 *                                  array incl. separate URL parts (optional).
	 *
	 * @return string|mixed[] Current URL or array (URL + separate parts).
	 */
	public static function get_current_url( $query_var_names = [], $mode = 'include', $url_only = true ) {
		$defaults = [
			'scheme' => 'https',
			'host'   => '',
			'port'   => '',
		...
		$url_parsed = array_merge(
			$defaults,
			wp_parse_url( home_url() ),
			wp_parse_url( add_query_arg( [] ) )
		);
		...
		if ( ! empty( $url_parsed['query'] ) ) {
			...
			if ( ! empty( $query_var_names ) ) {
				if ( ! is_array( $query_var_names ) ) {
					$query_var_names = [ $query_var_names ];
				}

				if ( 'include' === $mode ) {
					$query_args = array_intersect_key( $query_args, array_flip( $query_var_names ) );
				} elseif ( 'exclude' === $mode ) {
					$query_args = array_diff_key( $query_args, array_flip( $query_var_names ) );
				}
			}

			$url_parsed['query_args'] = $query_args;
			$url_parsed['query']      = http_build_query( $query_args );
		}

		$url_parsed['url'] = $url_parsed['scheme'] . '://' .
			$url_parsed['host'] .
			( ! empty( $url_parsed['port'] ) ? ':' . $url_parsed['port'] : '' ) .
			( $url_parsed['path'] ? trailingslashit( $url_parsed['path'] ) : '/' ) .
			( ! empty( $url_parsed['query'] ) ? '?' . $url_parsed['query'] : '' );

		return $url_only ? $url_parsed['url'] : $url_parsed;
	} // get_current_url

	/**
	 * Return the current page URL based on home_url() and public query variables
	 * excluding /page/X and GET variables with the given names.
	 *
	 * @since 0.9
	 *
	 * @param bool     $separate_query_vars True if an array with URL and separate
	 *                                      query vars shall be returned.
	 * @param string[] $exclude             Query variable names to exclude (optional,
	 *                                      default: "page" and "paged").
	 *
	 * @return string|mixed[] Current URL without page number or
	 *                        array (URL + vars).
	 */
	public static function get_nopaging_url( $separate_query_vars = false, $exclude = [ 'page', 'paged' ] ) {
		global $wp;

		$public_query_vars = array_diff( $wp->public_query_vars, $exclude );
		$current_url       = self::get_current_url( $public_query_vars, 'include', false );
		$nopaging_url      = preg_replace( '/page\\/[0-9]+(\\/)?/i', '', $current_url['url'] );
	...
	/**
	 * Extract the page number – if any – from the given URL.
	 *
	 * @since 2.12.0
	 *
	 * @param string   $url           URL.
	 * @param int|bool $default_value Default value (optional).
	 *
	 * @return int|bool Page number or default value if nonexistent.
	 */
	public static function get_page_num_from_url( $url, $default_value = false ) {
		$url_parts = wp_parse_url( $url );

		if (
			( ! get_option( 'permalink_structure' ) || preg_match( '/[?&]paged=[0-9]/', $url ) )
			...
			$found = preg_match( '/\/page\/([0-9]+)(?=\/|$)/', $url, $matches );
			return $found ? (int) $matches[1] : $default_value;
		}
	} // get_page_num_from_url

	/**
	 * Update or remove the page number in a given URL.
	 *
	 * @since 2.12.0
	 *
	 * @param string $url         URL.
	 * @param int    $page_number Page number to set (0 or 1 to remove).
	 *
	 * @return string Possibly updated URL.
	 */
	public static function update_url_pagination( $url, $page_number ) {
		$url_parts = wp_parse_url( $url );
		$path      = ! empty( $url_parts['path'] ) ? $url_parts['path'] : '';

		if (
			( ! get_option( 'permalink_structure' ) || preg_match( '/[?&]paged=[0-9]/', $url ) )
			&& false === strpos( $path, '/page/' )
		) {
			$query_args = [];
			...
			return $url_split[0] . ( $query ? "?{$query}" : '' );
		} elseif ( false !== strpos( $path, '/page/' ) ) {
			$page_replace_part = (int) $page_number >= 2 ? "/page/{$page_number}/" : '/';

			return preg_replace( '/\/page\/[0-9]+\/?(?=\?|$)/', $page_replace_part, $url );
		} elseif (
			$page_number > 1
			&& false === strpos( $path, '/page/' )
		) {
			$replace = trailingslashit( $url_parts['host'] . $path );
			$by      = untrailingslashit( $replace ) . "/page/{$page_number}/";

			return str_replace( $replace, $by, $url );
		}

		return $url;
	} // update_url_pagination
	 ...
	 * @since 1.1.0
	 *
	 * @param string $source String containing the mail addresses.
	 * @param string $divider String/Character used to separate addresses.
	 *
	 * @return string[] Extracted mail addresses.
	 */
	public static function split_mail_address_string( $source, $divider = ',' ) {
		$mail_addresses = [];
		$string_parts   = explode( $divider, $source );
	 ...
	 * @since 1.1.0
	 *
	 * @param string $source String to encrypt/decrypt.
	 * @param string $key    Encryption/Decryption key.
	 *
	 * @return string Encrypted string.
	 */
	public static function xor_string( $source, $key ) {
		$len = strlen( $source );

		for ( $i = 0; $i < $len; $i++ ) {
			$source[ $i ] = ( $source[ $i ] ^ $key[ $i % strlen( $key ) ] );
		}
	 ...
	 */
	public static function html_to_plain_text( $html, $list_bullet = '', $options = [] ) {
		$options = array_merge(
			[ 'add_colon' => true ],
			$options
		);
	...
	public static function doc_link( $url, $link_text = '', $dashicon = 'info-outline' ) {
		if ( ! in_array( $dashicon, [ 'info-outline', 'info', 'help', 'book', 'sos' ], true ) ) {
	...
	public static function unify_dirsep( $path, $trailingslashit = 0, $dirsep = DIRECTORY_SEPARATOR ) {
		...
		return str_replace( [ '/', '\\' ], $dirsep, $path );
	} // unify_dirsep
	 ...
	 * @param string $code_class Optional CSS class name for the code tag
	 *                           (defaults to immonex-qformat-source)
	 *
	 * @return string Rendered HTML output.
	 */
	public static function quick_format_source_code( $source, $code_class = 'immonex-qformat-source' ) {
	 ...
	 */
	public static function split_list_string( $source, $mode = 'auto', $valid_chars = false, $divider_chars = '=:', $convert_bool = true ) {
		if (
			! is_string( $source )
		...
		foreach ( $matches as $match ) {
			if ( 1 === preg_match( '/\|-([A-Z0-9-]+)?-\|/', $match['key'] ) ) {
				continue;
			}
			...
			if ( isset( $match['list'] ) ) {
				...
				$value    = self::split_list_string( $value, $sub_mode, $valid_chars );
			}
	...
	/**
	 * Generate a short, fairly unique and URL-safe ID based on the specified
	 * source string.
	 *
	 * @since 2.1.6
	 *
	 * @param string $source Source string.
	 * @param int    $length Max. hash length (optional, default: 8).
	 *
	 * @return string Generated hash.
	 */
	public static function get_hash( $source, $length = 8 ) {
		// phpcs:ignore
		$hash_b64     = base64_encode( hash( 'sha256', $source, true ) );
		$hash_urlsafe = strtr( $hash_b64, '+/', '-_' );
