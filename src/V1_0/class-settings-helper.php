...
class Settings_Helper {
	...
	/** @var mixed[] Tabs to display on the plugin options page */
	private $option_page_tabs = array();

	/** @var mixed[] Array for storing related "page" url fragments of options sections */
	private $section_page = array();

	/** @var mixed[] Sections to display inside the options page tabs */
	private $sections = array();

	/** @var mixed[] Input elements to display inside the options sections */
	private $fields = array();

	/** @var string Current options page tab */
	private $current_tab;
	...
	public function plugin_settings_link( $links ) {
		if ( empty( $this->plugin_infos['settings_page'] ) ) return $links;
	...
	public function get_current_tab() {
		if ( isset( $_REQUEST['tab'] ) && $_REQUEST['tab'] ) return $_REQUEST['tab'];
		else return false;
	} // get_current_tab
	...
	public function register_extension_tabs( $extension_plugin_slug, $tabs ) {
		if ( $extension_plugin_slug !== $this->plugin_slug ) return;
	...
	public function render_page( $args = array() ) {
		$option_page_template = apply_filters( $this->plugin_slug . '_option_page_template',
			file_exists( $this->plugin_dir . '/partials/plugin_options.php' ) ?
				$this->plugin_dir . '/partials/plugin_options.php' :
				__DIR__ . '/partials/plugin_options.php'
		);
		...
		if ( count( $this->option_page_tabs ) > 0 ) {
			...
			if ( isset ( $_GET['tab'] ) && in_array( $_GET['tab'], array_keys( $this->option_page_tabs ) ) ) $this->current_tab = $_GET['tab'];
			else $this->current_tab = $option_page_tab_keys[0];
	...
	private function render_checkbox_group( $args ) {
		if ( ! isset( $args['options'] ) || 0 === count( $args['options'] ) ) return;

		if ( ! isset( $args['wrap'] ) ) $args['wrap'] = '<div>{element}</div>';
