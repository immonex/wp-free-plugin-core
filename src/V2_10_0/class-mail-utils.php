...
class Mail_Utils {
	...
	/**
	 * Register mail related hooks.
	 *
	 * @since 2.6.2
	 */
	public function init() {
		$plugin_prefix = $this->bootstrap_data['plugin_prefix'];

		add_filter( "{$plugin_prefix}send_mail", [ $this, 'send_mail' ], 10, 7 );
		add_filter( "{$plugin_prefix}send_admin_mail", [ $this, 'send_admin_mail' ], 10, 7 );
	} // init

	/**
	 * Send a mail (filter callback).
	 *
	 * @since 2.6.2
	 *
	 * @param bool            $success       True if the mail request was processed without errors.
	 ...
	 */
	public function send_mail( $success, $to, $subject, $body, $headers = [], $attachments = [], $template_data = [] ) {
		return $this->send( $to, $subject, $body, $headers, $attachments, $template_data );
	} // send_mail

	/**
	 * Send an admin mail (filter callback).
	 *
	 * @since 2.6.2
	 *
	 * @param bool            $success       True if the mail request was processed without errors.
	 ...
	 * @return bool True on successful mail processing.
	 */
	public function send_admin_mail( $success, $to, $subject, $body, $headers = [], $attachments = [], $template_data = [] ) {
		if ( empty( $template_data['preset'] ) ) {
	 ...
	 * @return bool True on successful mail processing.
	 */
	public function send( $to, $subject, $body, $headers = [], $attachments = [], $template_data = [] ) {
		...
		if ( $body_html ) {
			...
			add_action( 'phpmailer_init', [ $this, 'phpmailer_set_alt_body' ] );
			$mail_result = wp_mail( $mail_data['to'], $mail_data['subject'], $body_txt, $mail_data['headers'], $mail_data['attachments'] );
			remove_action( 'phpmailer_init', [ $this, 'phpmailer_set_alt_body' ] );
