...
class Settings_Helper {
	...
	public function render_field( $args ) {
		...
		if (
			! empty( $args['condition'] )
			&& is_callable( $args['condition'] )
		) {
			$render_check = $args['condition']();
			if ( ! $render_check ) {
				return;
			}

			if ( empty( $render_check['is_active'] ) ) {
				$field_rendered = $this->render_conditional_field_content( $args, $render_check );
			}
		}
		...
		if ( ! empty( $args['special_info'] ) ) {
			if ( is_string( $args['special_info'] ) ) {
				$content = $args['special_info'];
				$type    = 'info';
			} else {
				$content = ! empty( $args['special_info']['content'] ) ? $args['special_info']['content'] : '';
				$type    = ! empty( $args['special_info']['type'] ) ? $args['special_info']['type'] : 'info';
			}
			...
			if ( $content ) {
				...
				echo wp_sprintf(
					'<div class="immonex-plugin-options__infobox immonex-plugin-options__infobox--is-%1$s">%2$s</div>' . PHP_EOL,
					$type,
					$content
				);
			}
	...
	private function render_media_image_select( $args ) {
		...
		$image_select = wp_sprintf(
			PHP_EOL . '<div id="%3$s-media-wrapper" class="immonex-plugin-options__media-wrapper">' .
				'%6$s</div>' .
				'<input id="%3$s-select-button" type="button" class="button" value="%5$s" data-media-frame-title="%7$s" data-media-frame-button-text="%8$s" data-max-files="%9$s">' .
			...
			$select_button_text,
			$image_preview,
			! empty( $args['media_frame_title'] ) ? $args['media_frame_title'] : '',
			! empty( $args['media_frame_button_text'] ) ? $args['media_frame_button_text'] : '',
			! empty( $args['max_files'] ) ? $args['max_files'] : '',
		);
	...
	/**
	 * Render the content of a field that is not active due to a conditional check.
	 *
	 * @since 2.10.0
	 *
	 * @param mixed[] $args      Field properties.
	 * @param mixed[] $cond_args Result of the condition callback.
	 *
	 * @return bool True if the field content has been rendered.
	 */
	public function render_conditional_field_content( $args, $cond_args ) {
		echo wp_sprintf(
			'<input id="%1$s" type="hidden" name="%2$s[%3$s]" value="%4$s" autocomplete="off">' . PHP_EOL,
			$args['id'],
			$args['option_name'],
			$args['name'],
			isset( $cond_args['value'] ) ? $cond_args['value'] : ''
		);

		if ( $cond_args['info'] ) {
			echo wp_sprintf(
				'<div class="immonex-plugin-options__conditional-info">%s</div>',
				$cond_args['info']
			);
		}

		return true;
	} // render_conditional_field_content
