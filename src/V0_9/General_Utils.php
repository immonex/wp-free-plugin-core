...
class General_Utils {
	...
	public static function reset_post( $post_id, $taxonomies = false, $overwrite_defaults = array(), $exclude_meta = array(), $special_args = array() ) {
		...
		do_action( 'inveris_base_before_post_reset', $post_id, array(
		...
		if ( isset( $special_args['keep_attachments'] ) && $special_args['keep_attachments'] ) $keep_attachments = true;
		else $keep_attachments = false;
		 ...
		 */
		if ( false === $taxonomies ) $taxonomies = get_taxonomies();
		...
		if ( ! $keep_attachments ) {
			...
			if ( $keep_featured_image ) $args['exclude'] = get_post_thumbnail_id( $post_id );
		...
		if ( $overwrite_defaults && is_array( $overwrite_defaults ) ) $post_defaults = array_merge( $post_defaults, $overwrite_defaults );
	 ...
	 * @param int Search flags (glob function).
	 *
