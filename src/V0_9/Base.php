<?php
/**
 * immonex WP Free Plugin Core provides shared basic functionality for free
 * immonex WordPress plugins.
...
abstract class Base {
	...
	const BASE_VERSION = '0.9';

	/** @var string Name of the custom field for storing plugin options */
	protected $plugin_options_name;

	/** @var mixed[] Plugin options */
	protected $plugin_options = array();

	/** @var string Minimun WP capability to access the plugin options page */
	protected $plugin_options_access_capability = 'manage_options';

	/** @var bool Does the plugin has its own options page? */
	protected $enable_separate_option_page = false;

	/** @var string Name of the Link that leads to the plugin's options page */
	protected $options_link_title = '';

	/** @var string Title (HTML head) of the plugin's options page (if any) */
	protected $options_page_title = '';

	/** @var string Plugin information and URLs for displaying in the options footer */
	protected $plugin_infos = array();

	/** @var string Priority for the init action (init_plugin method) */
	protected $init_plugin_priority;

	/** @var string[] Set of core plugin data (name, slug, version etc.) */
	protected $bootstrap_data = array();

	/** @var mixed[] Admin notices to display */
	protected $admin_notices = array();

	/** @var bool Translations already loaded? */
	protected $translations_loaded = false;

	/** @var string Plugin slug */
	public $plugin_slug;

	/** @var string Gettext textdomain of plugin translations */
	public $textdomain;

	/** @var string Plugin directory (full path) */
	public $plugin_dir;

	/** @var string Main plugin file (full path) */
	public $plugin_main_file;

	/** @var string Handle for enqueuing the main backend CSS file */
	public $backend_css_handle;

	/** @var string Handle for enqueuing the main backend JS file */
	public $backend_js_handle;

	/** @var string Handle for enqueuing the main frontend CSS file */
	public $frontend_base_css_handle;

	/** @var string Handle for enqueuing the main frontend JS file */
	public $frontend_base_js_handle;

	/** @var \immonex\WordPressFreePluginCore\V0_9\Settings_Helper Utility object */
	public $settings_helper;

	/** @var \immonex\WordPressFreePluginCore\V0_9\General_Utils Utility object */
	public $general_utils;

	/** @var \immonex\WordPressFreePluginCore\V0_9\String_Utils Utility object */
	public $string_utils;

	/** @var \immonex\WordPressFreePluginCore\V0_9\Geo_Utils Utility object */
	public $geo_utils;

	/** @var \immonex\WordPressFreePluginCore\V0_9\Template_Utils Utility object */
	public $template_utils;

	/** @var object[] Set of all utility class instances mentioned above */
	public $core_utils;

	/** @var object[] Additional utility objects (will be merged with core utils) */
	public $utils = array();

	/** @var \WP_Filesystem_Base WP_Filesystem object */
	public $wp_filesystem;

	/** @var bool Has the plugin been activated network-wide? */
	...
	public function activate_plugin() {
		...
		if ( $this->plugin_options['plugin_version'] !== static::PLUGIN_VERSION ) {
		...
		if ( ! wp_get_schedule( static::PLUGIN_PREFIX . 'do_daily' ) ) wp_schedule_event( time(), 'daily', static::PLUGIN_PREFIX . 'do_daily' );
		if ( ! wp_get_schedule( static::PLUGIN_PREFIX . 'do_weekly' ) ) wp_schedule_event( time(), 'weekly', static::PLUGIN_PREFIX . 'do_weekly' );
	} // activate_plugin
	...
	public function init_base() {
		...
		$this->enable_separate_option_page = apply_filters(
			static::PLUGIN_PREFIX . 'enable_separate_option_page',
	...
	public function init_plugin() {
		...
		$this->plugin_options_access_capability = apply_filters(
			static::PLUGIN_PREFIX . 'plugin_options_access_capability',
			$this->plugin_options_access_capability
		);
	...
	public function display_admin_notices() {
		if ( count( $this->admin_notices ) > 0 ) {
			...
			foreach ( $this->admin_notices as $id => $notice ) {
				if ( $notice['is_dismissable'] ) $dismissables_cnt++;
				if ( $dismissables_cnt > 1 ) break;
			}
			...
			foreach ( $this->admin_notices as $id => $notice ) {
				...
				if ( $notice['is_dismissable'] ) {
					...
					if ( $dismissables_cnt > 1 ) $message .= wp_sprintf( ' | <a href="%s?dismiss_inveris_admin_notice=1" style="text-decoration:underline">%s</a>', $admin_url, __( 'Dismiss all', 'immonex-wp-free-plugin-core' ) );
				}
	...
	protected function add_deferred_admin_notice( $message, $type = 'updated' ) {
		...
		if ( count( $this->plugin_options['deferred_admin_notices'] ) ) {
			foreach ( $this->plugin_options['deferred_admin_notices'] as $notice ) {
				if ( $notice['message'] === $message ) return;
			}
		}
	...
	protected function load_translations() {
		if ( $this->translations_loaded ) return;
