<?php
/**
 * This file contains a utility class for color calculations.
 */
...
class Color_Utils {
	...
	public function get_luminance_difference( $hex1, $hex2 ) {
		...
		if ( ! $luminance1 ) return false;

		$luminance2 = $this->get_luminance( $hex2 );
		if ( ! $luminance2 ) return false;
	...
	public function adjust_luminance( $hex, $percent_dec ) {
		...
		if ( $percent_dec < -100 || $percent_dec > 100 ) return $hex;
	...
	public function get_brightness_difference( $hex1, $hex2 ) {
		...
		if ( ! $brightness1 ) return false;

		$brightness2 = $this->get_brightness( $hex2 );
		if ( ! $brightness2 ) return false;
	...
	public function rgb2hex( $rgb ) {
		if ( ! is_array( $rgb ) || 3 !== count( $rgb ) ) return false;
	...
	public function expand_hex_color_value( $hex, $hash = true ) {
		...
		if ( 3 !== strlen( $hex ) && 6 !== strlen( $hex ) ) return false;
