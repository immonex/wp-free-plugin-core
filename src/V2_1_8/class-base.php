...
namespace immonex\WordPressFreePluginCore\V2_1_8;
...
abstract class Base {
	...
	public function __construct( $plugin_slug, $textdomain = false, $plugin_fs_dir = false ) {
		...
		add_action( 'immonex_core_after_activation', array( $this, 'maybe_update_plugin_version' ), 20 );
		add_action( 'immonex_core_after_activation', array( $this, 'perform_plugin_option_cleanup' ), 30 );
	...
	public function init( $init_plugin_priority = 10 ) {
		...
		add_action( 'switch_locale', array( $this, 'reload_translations_on_switch' ) );
	} // init
	...
	public function clean_up_plugin_options( $save = false ) {
		...
		return ! empty( $options['outdated'] ) ? $options['outdated'] : array();
