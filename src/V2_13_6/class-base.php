...
namespace immonex\WordPressFreePluginCore\V2_13_6;
...
abstract class Base {
	 ...
	 * @var mixed[]
	 */
	protected $default_plugin_options = [];
	...
	public function __construct( $plugin_slug, $textdomain = false, $plugin_fs_dir = false ) {
		if ( $plugin_slug ) {
			...
			$this->plugin_prefix          = defined( 'static::PLUGIN_PREFIX' ) ? static::PLUGIN_PREFIX : '';
		...
		add_filter( "{$this->plugin_slug}_plugin_infos", [ $this, 'get_plugin_infos' ] );
		...
		register_activation_hook( $this->plugin_main_file, [ $this, 'activate_plugin' ] );
		register_deactivation_hook( $this->plugin_main_file, [ $this, 'deactivate_plugin' ] );
		...
		add_action( static::PLUGIN_PREFIX . 'update_plugin_options', [ $this, 'update_plugin_options' ], 10, 2 );
		add_action( static::PLUGIN_PREFIX . 'add_deferred_admin_notice', [ $this, 'add_deferred_admin_notice' ], 10, 4 );
		add_action( static::PLUGIN_PREFIX . 'dismiss_deferred_admin_notice', [ $this, 'dismiss_admin_notice' ], 10 );
		add_action( static::PLUGIN_PREFIX . 'admin_mail', [ $this, 'send_admin_mail' ], 10, 6 );

		add_action( 'immonex_core_after_activation', [ $this, 'maybe_update_plugin_version' ], 20 );
		...
		add_filter( 'immonex_options', [ $this, 'get_combined_options' ], 30, 2 );
		add_filter( 'inx_options', [ $this, 'get_combined_options' ], 30, 2 );
	...
	public function __get( $key ) {
		...
		switch ( $key ) {
			...
			case 'plugin_infos':
				// phpcs:ignore
				$value = apply_filters( static::PLUGIN_PREFIX . '_plugin_infos', $this->plugin_infos );
				break;
			case 'cpt_hooks':
	...
	protected function activate_plugin_single_site( $fire_before_hook = true, $fire_after_hook = true ) {
		if ( $fire_before_hook ) {
			do_action( 'immonex_core_before_activation', $this->plugin_slug );
		}
		...
		if ( $this->enable_separate_option_page && $options_link_menu_location[0] ) {
			if ( 'settings' === $options_link_menu_location[0] ) {
				...
				add_options_page(
					...
					[ $this->settings_helper, 'render_page' ]
				);
			} else {
	...
	public function init( $init_plugin_priority = 10 ) {
		...
		add_action( 'plugins_loaded', [ $this, 'init_base' ] );
		add_action( 'after_setup_theme', [ $this, 'load_translations' ] );
		add_action( 'switch_locale', [ $this, 'reload_translations_on_switch' ] );
	} // init
	...
	public function init_base() {
		...
		add_action( 'init', [ $this, 'init_plugin' ], $this->init_plugin_priority );
		add_action( 'widgets_init', [ $this, 'init_plugin_widgets' ] );
		add_action( 'admin_init', [ $this, 'init_plugin_admin' ] );
		add_action( 'admin_menu', [ $this, 'register_plugin_settings' ], $options_link_menu_location[1] );

		/**
		 * Add filters for modifying the required user/role capability for accessing
		 * and updating plugin options.
		 */
		add_filter( "{$this->plugin_slug}_plugin_options_access_capability", [ $this, 'get_default_plugin_options_access_capability' ] );
		add_filter( "option_page_capability_{$this->plugin_options_name}", [ $this, 'get_plugin_options_access_capability' ] );
		...
		if ( ! isset( $this->default_plugin_options['deferred_admin_notices'] ) ) {
			$this->default_plugin_options['deferred_admin_notices'] = [];
		}
	} // init_base
	...
	public function init_plugin( $fire_before_hook = true, $fire_after_hook = true ) {
		...
		if (
			...
			version_compare( $this->plugin_options['plugin_version'], static::PLUGIN_VERSION, 'ne' )
		) {
		...
		add_action( static::PLUGIN_PREFIX . 'do_daily', [ $this, 'do_daily' ] );
		add_action( static::PLUGIN_PREFIX . 'do_weekly', [ $this, 'do_weekly' ] );
		...
		$this->plugin_cache = new Plugin_Cache( static::PLUGIN_PREFIX );
		$this->plugin_cache->init();

		$this->cache_buster = new Cache_Buster( $this->plugin_slug );
		$this->cache_buster->init();
	...
	public function init_plugin_admin( $fire_before_hook = true, $fire_after_hook = true ) {
		...
		add_action( 'admin_enqueue_scripts', [ $this, 'admin_cleanup_scripts_and_styles' ], 100 );

		if ( $is_admin ) {
			add_action( 'network_admin_notices', [ $this, 'display_network_admin_notices' ] );
		...
		if ( $this->settings_page ) {
			add_filter(
				...
				[ $this->settings_helper, 'plugin_settings_link' ]
			);
		}
	...
	protected function register_cpt_hooks( $component_config ) {
		...
		foreach ( $this->bootstrap_data['custom_post_types'] as $cpt_base_name => $cpt ) {
			foreach ( [ '_Hooks', '_List_Hooks' ] as $class_name_suffix ) {
	...
	private function init_utils() {
		...
		$this->core_utils = [
			...
			'domain'    => $this->domain_utils,
			'video'     => $this->video_utils,
			'embed'     => $this->embed_utils,
			'array'     => $this->array_utils,
		];
		...
		add_filter( 'immonex_core_template_data', [ $this->template_utils, 'add_default_template_data' ], 90, 2 );
	...
	public function frontend_scripts_and_styles() {
		$css_search_folders = [ 'assets/css', 'assets', 'css' ];
		$js_search_folders  = [ 'assets/js', 'assets', 'js' ];
		...
		$default_skin_css_handle = static::PUBLIC_PREFIX . 'default-skin';
		$default_skin_css_url    = '';

		// phpcs:ignore
		if ( apply_filters( "{$this->plugin_prefix}use_default_skin_assets_as_fallback", true ) ) {
			// Always enqueue default skin main CSS file first if not disabled via filter function.
			$default_skin_css_url = plugins_url( "{$this->plugin_slug}/skins/default/assets/index.css" );
			$required             = wp_style_is( $this->frontend_base_css_handle ) ?
				[ $this->frontend_base_css_handle ] : [];

			wp_enqueue_style(
				$default_skin_css_handle,
				$default_skin_css_url,
				$required,
				$this->plugin_version
			);
		...
		if ( ! empty( $this->plugin_options['skin'] ) ) {
			$lookup_basenames = [ 'index', 'extend', 'custom', 'frontend', 'skin' ];
			...
			foreach ( $lookup_basenames as $basename ) {
				...
				if ( $skin_css ) {
					...
					if ( $skin_css_url && $skin_css_url !== $default_skin_css_url ) {
						$skin_css_deps = [];
						if ( wp_style_is( $this->frontend_base_css_handle ) ) {
							$skin_css_deps[] = $this->frontend_base_css_handle;
						}
						if ( wp_style_is( $default_skin_css_handle ) ) {
							$skin_css_deps[] = $default_skin_css_handle;
						}

						$handle = static::PUBLIC_PREFIX . ( 'index' === $basename ? 'skin' : $basename );
	...
	public function fetch_plugin_options( $defaults = false ) {
		...
		$default_options = $defaults && is_array( $defaults ) ? $defaults : $this->default_plugin_options;

		if ( ! is_array( $plugin_options ) ) {
			return $default_options;
		}

		return array_merge( $default_options, $plugin_options );
	} // fetch_plugin_options

	/**
	 * Perform plugin option cleanup and optional modification by filter
	 * functions (action callback).
	 *
	 * @since 1.9.13
	 *
	...
	public function perform_plugin_option_cleanup( $plugin_slug ) {
		...
		if ( $options !== $this->plugin_options ) {
			$this->plugin_options = $options;
			update_option( $this->plugin_options_name, $options );
		}
	} // perform_plugin_option_cleanup

	/**
	 * Remove and return OUTDATED elements from current plugin options.
	 *
	 * @since 1.9.13
	 *
	 * @param bool $save Flag to indicate if cleaned options should
	 *                   be saved (optional).
	 *
	 * @return mixed[] Removed plugin options.
	 */
	public function clean_up_plugin_options( $save = false ) {
		...
		$options = [
			'current'  => $this->plugin_options,
			'outdated' => [],
		];

		foreach ( $options['current'] as $key => $value ) {
			if ( ! in_array( $key, $ignore, true ) && ! isset( $this->default_plugin_options[ $key ] ) ) {
				unset( $options['current'][ $key ] );
				$options['outdated'][ $key ] = $value;
			}
		}

		if ( empty( $options['outdated'] ) ) {
			return [];
		}

		if ( $save ) {
			// phpcs:ignore
			$options = apply_filters( "{$this->plugin_slug}_remove_outdated_plugin_options", $options );

			$this->plugin_options = $options['current'];
			update_option( $this->plugin_options_name, $options['current'] );
		}

		return ! empty( $options['outdated'] ) ? $options['outdated'] : [];
	} // clean_up_plugin_options

	/**
	 * Update version information in plugin options if changed (action callback).
	 *
	 * @since 1.9.13
	 *
	...
	} // maybe_update_plugin_version

	/**
	 * Update changed plugin options (action callback ONLY).
	 *
	 * @since 1.2.2
	 *
	...
	public function admin_scripts_and_styles( $hook_suffix ) {
		...
		$core_version_folder = basename( __DIR__ );
		$core_version_handle = str_replace( '.', '-', static::CORE_VERSION );
		$core_handle         = "immonex-backend-core-{$core_version_handle}";

		/**
		 * Enqueue core backend CSS.
		 */
		wp_enqueue_style(
			$core_handle,
		 ...
		 */
		$base_css_folder = '';
		foreach ( [ 'assets/css', 'assets', 'css' ] as $folder ) {
		 ...
		 */
		$base_js_folder = '';
		foreach ( [ 'assets/js', 'assets', 'js' ] as $folder ) {
	...
	/**
	 * Dequeue deprecated CSS/JS files (callback).
	 *
	 * @since 2.7.0
	 *
	 * @param string $hook_suffix The current admin page.
	 */
	public function admin_cleanup_scripts_and_styles( $hook_suffix ) {
		global $wp_styles;
		global $wp_scripts;

		$highest_versions_queued = [
			'styles'  => [],
			'scripts' => [],
		];
		...
		foreach ( array_keys( $highest_versions_queued ) as $type ) {
			if ( ! empty( ${"wp_{$type}"}->queue ) ) {
				foreach ( ${"wp_{$type}"}->queue as $i => $handle ) {
					if ( preg_match( '/^immonex-(?:[a-z0-9-]+)?backend-core-([0-9-]+)$/', $handle, $matches ) ) {
						$version       = str_replace( '-', '.', $matches[1] );
						$major_version = explode( '.', $version )[0];

						if (
							! isset( $highest_versions_queued[ $type ][ $major_version ] )
							|| version_compare( $version, $highest_versions_queued[ $type ][ $major_version ]['version'], '>' )
						) {
							if ( isset( $highest_versions_queued[ $type ][ $major_version ]['handle'] ) ) {
								$dequeue_handles[ $type ][] = $highest_versions_queued[ $type ][ $major_version ]['handle'];
							}

							$highest_versions_queued[ $type ][ $major_version ]['version'] = $version;
							$highest_versions_queued[ $type ][ $major_version ]['handle']  = $handle;
						} else {
							$dequeue_handles[ $type ][] = $handle;
						}
					}
				}
			}
		...
		foreach ( $dequeue_handles as $type => $handles ) {
			if ( ! empty( $handles ) ) {
	...
	} // admin_cleanup_scripts_and_styles
		...
		if ( is_array( $tab_fields ) && count( $tab_fields ) > 0 ) {
			foreach ( $tab_fields as $name => $field ) {
				...
				if ( $exists ) {
					...
					$value = $field['default'];
				} elseif ( ! in_array( $field['type'], [ 'checkbox', 'checkbox_group' ], true ) ) {
					continue;
				}
	...
	protected function get_plugin_footer_infos() {
		$infos = [];
		$name  = defined( 'static::ADDON_NAME' ) && static::ADDON_NAME
			? static::ADDON_NAME . ' Add-on'
			: str_replace( 'immonex ', '', static::PLUGIN_NAME );

		if ( defined( 'static::PLUGIN_VERSION' ) ) {
			$display_version = '<strong>' . static::PLUGIN_VERSION . '</strong>';
	...
	public function add_deferred_admin_notice( $message, $type = 'success', $context = '', $id = '' ) {
		...
		if ( ! in_array( $raw_type, [ 'success', 'info', 'warning', 'error' ], true ) ) {
			$type = 'info';
		}
		...
		$this->plugin_options['deferred_admin_notices'][ $id ? $id : uniqid() ] = [
			'message' => $message,
			'type'    => $type,
		];
	 ...
	 */
	public function send_admin_mail( $subject, $body, $headers = [], $attachments = [], $template_data = [], $to = false ) {
		if ( empty( $to ) ) {
	...
	/**
	 * Reload translations before locale switch (action callback).
	 *
	 * @since 1.9.7
	 *
	 * @param string    $locale           The new locale.
	 * @param false|int $user_id $user_id User ID for context if available (optional).
	 */
	public function reload_translations_on_switch( $locale, $user_id = false ) {
		$this->load_translations( true );
	} // reload_translations_on_switch
	...
	/**
	 * Return combined current options of multiple immonex/kickstart plugins and
	 * add-ons (filter callback).
	 *
	 * @since 2.4.5
	 *
	 * @param mixed[] $options    Source options or empty array.
	 * @param string  $plugin_key Return options with the specified key only (optional).
	 *
	 * @return mixed[] Extended option array with one sub-array per plugin or
	 *                 flat array if a specific plugin key is given.
	 */
	public function get_combined_options( $options, $plugin_key = '' ) {
		$key = preg_replace( '/^(inx|immonex)_([a-z_]+)/', '$2', trim( $this->plugin_prefix, '_' ) );

		if ( 'inx' === $key ) {
			// immonex Kickstart Core Plugin.
			$key = 'core';
		}
		...
		// (Re)fetch current plugin options.
		$plugin_options       = $this->fetch_plugin_options();
		$this->plugin_options = $plugin_options;

		if ( isset( $plugin_options['deferred_tasks'] ) ) {
			unset( $plugin_options['deferred_tasks'] );
		}
	 ...
	 * @since 0.1
	 */
	public function load_translations( $force_reload = false ) {
		...
		$locales = array_filter( [ $locale, $fallback_locale ] );

		/**
		 * Load plugin base translations first.
		 */
		foreach ( $locales as $try_locale ) {
			...
			if ( load_textdomain( 'immonex-wp-free-plugin-core', $local_mo_file ) ) {
				break;
			}
		}
		...
		if ( $this->is_stable || $always_load_global_translations ) {
			...
			foreach ( $locales as $try_locale ) {
				...
				if ( load_textdomain( $this->textdomain, $local_mo_file ) ) {
					break;
				}
			}
		}
