...
class Template_Utils {
	 ...
	 * @param string          $add_folder_mode Where to insert the additional folders (optional, before, after or override).
	 * @param string|bool     $force_skin Temporary use the given skin instead of the main one (optional).
	 *
	...
	public function locate_template_file( $filenames, $add_folders = [], $add_folder_mode = 'before', $force_skin = false ) {
		...
		$valid_folders = $this->get_valid_template_folders( $add_folders, $add_folder_mode, $force_skin );

		foreach ( $filenames as $i => $filename ) {
			// Remove any dots with trailing directory separators for security reasons.
			$filenames[ $i ] = preg_replace( '/([.]+\\' . DIRECTORY_SEPARATOR . ')+/', '', $filename );
			...
			if (
				...
				&& in_array( dirname( $filename ), $valid_folders, true )
		...
		foreach ( $filenames as $filename ) {
			...
			if ( preg_match( '/[\[]?-([a-z]{2}(_[A-Z]{2})?(_(in)?formal)?)[\]]?\.' . $ext_regex . '/', $filename, $matches ) ) {
				...
				$localized_filenames = array_merge(
					$localized_filenames,
					[ str_replace( $matches[0], "-{$locale}", $filename ) ]
				);
	...
	public function get_default_template_folders( $reverse_order = false, $include_skin_folders = true, $existing_only = false ) {
		...
		$base_folders = apply_filters(
			...
			array_unique( $default_base_folders )
		);
		...
		if ( $include_skin_folders ) {
			...
			// phpcs:ignore
			if ( apply_filters( "{$this->plugin->plugin_prefix}use_default_skin_assets_as_fallback", true ) ) {
				$skin_folders[] = trailingslashit( $this->plugin->plugin_dir ) . 'skins/default';
			}
		}
	...
	/**
	 * Compose a list of all valid template folders (absolute paths).
	 *
	 * @since 2.4.2
	 *
	 * @param string[]    $add_folders Additional search folders (absolute paths).
	 ...
	 * @param string|bool $force_skin Temporary use the given skin instead of the main one (optional).
	 *
	 * @return string[] Absolute paths of valid template directories.
	 */
	private function get_valid_template_folders( $add_folders = [], $add_folder_mode = 'before', $force_skin = false ) {
		...
		if (
			...
			$valid_folders = $add_folders;
		} else {
			$valid_folders = $this->get_default_template_folders();
		}

		$valid_folders[] = __DIR__ . '/templates';
		$valid_folders[] = __DIR__ . '/partials';
		...
		if (
			...
			if ( 'before' === $add_folder_mode ) {
				$valid_folders = array_merge( $add_folders, $valid_folders );
			} elseif ( 'after' === $add_folder_mode ) {
				$valid_folders = array_merge( $valid_folders, $add_folders );
			}
		}

		$valid_folders = array_filter(
			$valid_folders,
			function ( $folder ) {
				return is_string( $folder ) && ! empty( $folder );
			}
		);
		...
		return array_unique( $valid_folders );
	} // get_valid_template_folders
